        ...
        <el-table ref="table" v-loading="crud.loading" :data="crud.data" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
          ...
          <el-table-column v-if="columns.visible('dept')" :show-overflow-tooltip="true" width="110" prop="dept" label="部门 / 岗位">
            <template slot-scope="scope">
              <div>{{ scope.row.dept?scope.row.dept.name+'/':'APP用户' }}{{ scope.row.job?scope.row.job.name:'' }}</div>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
              <udOperation
                ...
                :disabled-dle="scope.row.id === user.id || !scope.row.dept"
                :disabled-edit="!scope.row.dept"
              />
            </template>
          </el-table-column>
        </el-table>
